...
import (
	"fmt"
	"git.sinostage.net/micro.service.go/core/security"
	"net/url"
	"strconv"
	"strings"
	"time"
...
func (this signUtil) SignTry(identity SignIdentity, signKey string, tryType int) string {
	...
	serviceSign := koloSecurity.MD5.ToHex(signKey + identity.Url.Path + md5String + strconv.FormatInt(identity.TS, 10))
